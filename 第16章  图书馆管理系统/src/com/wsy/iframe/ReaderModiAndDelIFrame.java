...
public class ReaderModiAndDelIFrame extends JInternalFrame {
	...
	class ModiButtonListener implements ActionListener {
		...
		public void actionPerformed(final ActionEvent e) {
			...
			int i=Dao.UpdateReader(id, readername.getText().trim(), sex, age.getText().trim(), zjnumber.getText().trim(), date.getText().trim(), maxnumber.getText().trim(), tel.getText().trim(), Double.valueOf(keepmoney.getText().trim()), zj, zy.getText().trim(), bztime.getText().trim(), ISBN.getText().trim());
